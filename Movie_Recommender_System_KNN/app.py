...
def movie_poster_fetcher(imdb_link):
    ...
    movie_poster_link = []
    movie_poster_link.append(imdb_dp.attrs['content'])
...
def get_movie_info(imdb_link):
    ...
    movie_descr = []
    movie_descr.append(imdb_content.attrs['content'])
...
def run():
    img1 = Image.open('Movie_Recommender_System_KNN/meta/logo.jpg')
    img1 = img1.resize((250,250),)
    st.image(img1,use_column_width=False)
    st.title("Movie Recommender System")
    st.markdown('''<h4 style='text-align: left; color: #d73b5c;'> Movie Recommendations From IMDB TOP-5000 Movies List </h4>''',
    ...
    if cat_op == category[0]:
        ...
        select_movie = st.selectbox('Select movie: (Recommendations Will Be Based On This Selection)', ['--Select--'] + movies)
        ...
        if dec == 'No':
            if select_movie == '--Select--':
                ...
                no_of_reco = st.slider('Number Of Movies You Want To Get Recommended:', min_value=5, max_value=20, step=1)
                genres = data[movies.index(select_movie)]
                test_points = genres
                table = KNN_Movie_Recommender(test_points, no_of_reco+1)
                table.pop(0)
                c = 0
                st.success('Some Of The Movies From IMDB-5000-TOP-LIST Recommendations, Have a Glance')
            ...
            if select_movie == '--Select--':
                ...
                table.pop(0)
                c = 0
                st.success('Some Of The Movies From IMDB-5000-List Recommendations, Have a Glance')
                for movie, link, ratings in table:
                    c += 1
                    st.markdown(f"({c})[ {movie}]({link})")
